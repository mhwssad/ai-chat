...
6. 重构 TUI 布局，使其从固定三栏仪表盘升级为更接近工作台式 Web 应用的信息架构。

### 非目标

- 不重写现有核心 `AppContainer` 分层设计
- 不替换当前 TUI 技术栈
- 不引入 WebView、浏览器内嵌页面或第二套前端运行时
...
- `Dashboard` 当前采用固定的 `sidebar / main / detail` 三栏布局，所有 Tab 被迫共享同一种空间分配。
- 当前侧栏混合了全局状态、导航和局部摘要，信息职责不单一。
- 当前详情区宽度固定且语义单一，不适合承载 `agent`、`rag` 这类需要阶段状态、结果预览和日志切换的复杂工作流。
- 当前底部命令提示区同时承担快捷键说明、状态反馈和输入模式提示，密度偏高，长期使用时扫描成本较高。
- 布局尚未针对窄终端和宽终端做出明显差异化策略，导致空间利用不稳定。
...
本次布局改造不引入 Web 技术栈，但会借鉴工作台式 Web 应用的布局思路，将当前 TUI 从“固定三栏展示页”调整为“以任务为中心的工作区”。
...
- 主要操作区优先，占据最大可用面积
- 检视器与详情区按需显示，而不是强制常驻
- 每类工作流使用匹配的信息结构，不要求所有 Tab 共享同一种三栏形态
- 在窄终端上优先保证可操作性，在宽终端上优先保证并行可见性

### 全局壳层

新的 `Dashboard` 应作为统一工作台壳层，拆分为以下区域：

- 顶部状态栏：应用名、当前会话、当前 Tab、后台任务状态、调度器状态、主题状态
- 左侧导航区：一级功能导航，仅展示 Tab 列表与少量全局计数
- 中央工作区：当前 Tab 的主操作区，承担列表、表单、对话流、结果流等主要内容
- 右侧检视区：按当前 Tab 语义显示详情、预览、日志、参数、对象属性，可折叠或在窄屏下隐藏
- 底部操作栏：当前 Tab 快捷键、输入模式提示、最近一次反馈消息

与当前实现相比，新的左侧区域不再承担复杂内容展示，只保留导航职责；原先塞在侧栏中的局部数据，应迁移到主工作区或右侧检视区。
...
适合 `tools`、`memory`、`scheduler`、`rag`、`image`、`tts` 等需要“列表 + 详情”并行查看的场景。
...
导航区和检视区均改为按需切换视图，不长期占据空间。

### 按工作流划分的布局模式

为避免所有 Tab 继续共享一种硬编码布局，`BaseTab` 需要支持声明自己的布局偏好。建议引入轻量布局描述对象，例如 `TabLayoutSpec`，由 `Dashboard` 根据其决定壳层排布。

建议的布局模式如下：

- `conversation`：适用于 `ChatTab`
- `execution`：适用于 `AgentTab`
- `resource`：适用于 `ToolsTab`、`MemoryTab`、`SchedulerTab`、`RagTab`
- `media`：适用于 `ImageTab`、`TTSTab`
- `system`：适用于 `StatsTab`、`SystemTab`
...
- 主工作区显示消息流
- 左侧显示会话列表
- 右侧显示当前会话上下文摘要、工具使用摘要或消息详情

对话流必须始终是视觉主轴，不允许被窄小的固定列压缩为次要内容。

#### execution 模式

用于 Agent 执行型工作流：

- 主工作区显示任务输入、当前阶段、执行结果
- 右侧检视区显示迭代日志、工具调用记录、异常信息
- 必要时支持在主区内部切换“任务编辑 / 运行中 / 结果”子视图
...
- 主工作区显示表格或列表
- 右侧检视区显示选中项详情、参数、日志或预览
- 上方或主区头部显示筛选条件、搜索状态和批量动作提示
...
- 主工作区显示资源清单
- 右侧检视区显示预览、元数据和操作状态
- 不在左侧区域堆叠媒体详情，避免与导航混淆

#### system 模式

用于系统状态与配置动作：

- 主工作区显示状态卡片、配置摘要、运行指标
- 右侧检视区显示某项配置或动作的详细说明、结果或风险提示

### 交互模型调整

为匹配新的布局结构，交互模型也需要同步调整：

- 将全局导航、Tab 内操作和对象级操作分层
- Tab 内搜索、筛选和创建动作优先放在主工作区头部，不再主要依赖底部提示记忆
- 右侧检视区支持显隐切换
- 对长任务场景，主工作区应保留清晰的状态位，避免仅在底部状态栏短暂提示

### 对 Dashboard 的结构要求

`Dashboard` 不应继续把布局写死为统一三栏。改造后应承担以下职责：

- 根据终端宽高选择布局档位
- 读取当前 Tab 的布局偏好
- 决定导航区、主工作区、检视区的可见性与比例
- 统一渲染全局状态栏和底部操作栏
- 为复杂 Tab 提供一致的工作台骨架，而不是让每个 Tab 自己拼壳层

### 对 Tab 的结构要求

每个 Tab 除现有内容渲染接口外，还应具备与布局协作的最小能力：
...
这样 `Dashboard` 控制壳层，`Tab` 控制业务内容，职责边界更清晰。
...
`Dashboard` 不再负责从全局容器读取依赖，也不再负责手工构造各个 Tab。同时，`Dashboard` 需要成为统一布局引擎，根据终端尺寸和 Tab 布局偏好决定壳层结构。
...
`BaseTab` 去掉运行时拉取线程池的逻辑，缓存刷新统一基于注入的线程池工作。除数据与交互外，`BaseTab` 还需要声明布局偏好，避免继续依赖 `Dashboard` 中针对固定索引的硬编码布局假设。
...
2. `Dashboard` 根据当前终端尺寸和 Tab 布局偏好构建壳层
3. `Tab` 基于缓存状态决定是否刷新
4. 若缓存过期，则使用注入的线程池在后台加载数据
5. Tab 通过已注入 service 获取业务数据
6. 渲染层仅消费已加载的数据，不再参与容器访问
...
- 为 `Dashboard` 与 `BaseTab` 引入布局偏好和自适应壳层所需接口
- 此阶段不改变功能入口，只先消除 CLI/TUI 内部直接抓全局容器的行为，并为布局重构建立骨架
...
- 宽终端下可同时看到导航区、主工作区、检视区
- 中等宽度终端下主工作区仍保持主要操作可读性
- 窄终端下不出现明显的内容重叠、截断或主流程不可操作
- `ChatTab` 与 `AgentTab` 的主内容区明显大于辅助区
- `RagTab`、`ToolsTab`、`MemoryTab`、`SchedulerTab` 的列表与详情职责分离清晰
...
- 布局引擎从固定三栏改为自适应后，若职责切分不清，容易让 `Dashboard` 和 `Tab` 再次相互耦合
...
- 由 `Dashboard` 统一控制壳层布局，`Tab` 仅声明布局偏好和业务内容，避免重复拼装页面框架
...
- TUI 布局从固定三栏升级为更贴近工作台的信息架构，能够根据任务类型和终端宽度调整展示方式
