...
class UnstructuredLoader(LoaderStrategy):
    ...
    Args:
        settings: UnstructuredSettings 配置实例，None 时使用全局单例。
        post_processors: 后处理函数列表，覆盖 settings 中的默认值。
    """

    def __init__(
        self,
        settings: UnstructuredSettings | None = None,
        *,
        post_processors: list[Any] | None = None,
    ) -> None:
        self._settings = settings or unstructured_settings
    ...
    def _enrich_metadata(
        self, docs: list[Document], file_path: Path
    ) -> list[Document]:
...
loader_registry.register(UnstructuredLoader, priority=100, name="unstructured")
