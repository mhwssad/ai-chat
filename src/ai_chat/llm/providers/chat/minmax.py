...
class MinMaxProvider(ChatProvider):
    ...
    通过 langchain-openai 包以 OpenAI 兼容协议对接 Minmax API。
    Minmax API 兼容 OpenAI 接口规范，因此复用 ChatOpenAI 客户端。

    支持模型：MiniMax-M2.7, minimax-m2.7
    """
    ...
    def __init__(self, config: Optional[ProviderConfig] = None) -> None:
        ...
        logger.debug("MinMaxProvider 初始化完成，base_url=%s", self._config.base_url)

    def get_client(self, model_name: str) -> BaseChatModel:
        """获取 Minmax LangChain 客户端实例（非流式，基于 OpenAI 兼容协议）。"""
        logger.debug("创建 MinMax 客户端: model=%s", model_name)
        return ChatOpenAI(
            model=model_name,
    ...
    def get_stream_client(self, model_name: str, *, temperature: float = 0.7, max_tokens: Optional[int] = None, stop: Optional[list[str]] = None) -> BaseChatModel:
        """获取带流式配置的 Minmax LangChain 客户端。"""
        logger.debug("创建 MinMax 流式客户端: model=%s, temperature=%.2f, max_tokens=%s",
    ...
    def chat(self, request: ChatRequest, model_name: str) -> ChatResponse:
        """发起 Minmax 聊天请求。"""
        logger.info("MinMax 聊天请求: model=%s, 消息数=%d, temperature=%.2f",
        ...
        logger.info("MinMax 聊天响应: model=%s, 内容长度=%d", model_name, len(content))
        return ChatResponse(
            content=content,
            model=model_name,
    ...
    def stream(self, request: ChatRequest, model_name: str, *, stop: Optional[list[str]] = None) -> Iterator[str]:
        """流式 Minmax 聊天，逐 token 返回文本片段。"""
        logger.info("MinMax 流式请求: model=%s, 消息数=%d", model_name, len(request.messages))
        ...
        for chunk in llm.stream(request.messages):
            if isinstance(chunk.content, str) and chunk.content:
                token_count += 1
                yield chunk.content
        logger.debug("MinMax 流式完成: model=%s, 共 %d 个 chunk", model_name, token_count)
