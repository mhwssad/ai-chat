...
from src.ai_chat.utils.http.converter import EntityConverter, register_converter
...
class JsonConverter(EntityConverter):
    ...
    def can_handle_request(self, data: object) -> bool:
        return isinstance(data, (dict, list)) or _is_pydantic_model(data) or _is_dataclass(data)
    ...
    def deserialize(
        ...
        if target_type is not None and isinstance(target_type, type) and issubclass(target_type, BaseModel):
